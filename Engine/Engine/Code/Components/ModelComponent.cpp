...
void ModelComponent::InitModel(char* model, unsigned int flags, wchar_t* diffuseMap, wchar_t* normalMap, wchar_t* specularMap, bool useMaterial, float tiling)
{
	_FLAGS = flags;
	...
	ProcessNode(scene->mRootNode, scene, diffuseMap, normalMap, specularMap, useMaterial, tiling);

	// get how many meshes that was loaded
	_numMeshes = _meshes.size();
}

void ModelComponent::InitPrimitive(PRIMITIVE_TYPE primitive, unsigned int flags, wchar_t* diffuseMap , wchar_t* normalMap , wchar_t* specularMap, float tiling)
{
...
{	
	...
	_meshes.push_back(ModelLoader::CreateGrid(size, cellSize, gridColor, flags, diffuseMap, normalMap, specularMap, tiling, _parent));
}
...
void ModelComponent::ProcessNode(aiNode* node, const aiScene* scene, wchar_t* diffuseMap, wchar_t* normalMap, wchar_t* specularMap, bool useMaterial, float tiling)
{
	// get and create all meshes in this node
	for (UINT i = 0; i < node->mNumMeshes; i++)
	{
		aiMesh* mesh = scene->mMeshes[node->mMeshes[i]];
		_meshes.push_back(ModelLoader::CreateMesh(mesh, scene, _FLAGS, diffuseMap, normalMap, specularMap, tiling, useMaterial, _parent));
	}

	// recursivly loop over and process all child nodes
	for (UINT i = 0; i < node->mNumChildren; i++)	
		ProcessNode(node->mChildren[i], scene, diffuseMap, normalMap, specularMap, useMaterial, tiling);	
}
