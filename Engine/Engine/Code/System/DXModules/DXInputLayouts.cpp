...
void DXInputLayouts::CreateInputLayout3DInstanced(ID3D10Blob*& vertexShaderByteCode)
{
	...
	HRESULT result;
	result = device->CreateInputLayout(inputLayout3DInstanced, 10, vertexShaderByteCode->GetBufferPointer(), vertexShaderByteCode->GetBufferSize(), &_inputLayout3DInstanced);
	if (FAILED(result))
		DX_ERROR::PrintError(result, "failed to create input layout 3D instanced");
}
